...
function default_compare(a: number, b: number): boolean {
  return a < b;
}
...
function std_sort(array: any[], first: number = 0, len: number = array.length - first, cmp: (a: any, b: any) => boolean = default_compare): any[] {
  ...
  for (; ;) { /* outer loop */
    for (; left + 1 < len; len++) { /* sort left to len-1 */
      let pivot = array[left + Math.floor(Math.random() * (len - left))];
      /* pick random pivot */
      stack[pos++] = len;
...
export class b2ParticleSystem {
  ...
  m_stuckParticleBuffer: b2GrowableBuffer<number> = new b2GrowableBuffer<number>(function () {
    return 0;
  });
  m_proxyBuffer: b2GrowableBuffer<b2ParticleSystem.Proxy> = new b2GrowableBuffer<b2ParticleSystem.Proxy>(function () {
    return new b2ParticleSystem.Proxy();
  });
  m_contactBuffer: b2GrowableBuffer<b2ParticleContact> = new b2GrowableBuffer<b2ParticleContact>(function () {
    return new b2ParticleContact();
  });
  m_bodyContactBuffer: b2GrowableBuffer<b2ParticleBodyContact> = new b2GrowableBuffer<b2ParticleBodyContact>(function () {
    return new b2ParticleBodyContact();
  });
  m_pairBuffer: b2GrowableBuffer<b2ParticlePair> = new b2GrowableBuffer<b2ParticlePair>(function () {
    return new b2ParticlePair();
  });
  m_triadBuffer: b2GrowableBuffer<b2ParticleTriad> = new b2GrowableBuffer<b2ParticleTriad>(function () {
